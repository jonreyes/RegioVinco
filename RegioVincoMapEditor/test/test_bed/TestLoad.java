...
public class TestLoad {
    ...
    public void testLoadData() throws Exception {
       System.out.println("FileManager Unit Test: Save Data");
        ...
        boolean equivalent = (preLoad.getBackgroundColor().equals(postLoad.getBackgroundColor()) &&
            preLoad.getBorderColor().equals(postLoad.getBorderColor()) &&
            preLoad.getBorderThickness() == postLoad.getBorderThickness() &&
            preLoad.getZoom() == postLoad.getZoom() &&
            
            preLoad.hasCapitals() == postLoad.hasCapitals() &&
            preLoad.hasFlags() == postLoad.hasFlags() &&
            preLoad.hasLeaders() == postLoad.hasLeaders() &&
            
            preLoad.getAnthem().equals(postLoad.getAnthem())) &&
            
            preLoad.getTableItems().equals(postLoad.getTableItems()) &&
            preLoad.getMapColors().equals(postLoad.getMapColors()) &&
            preLoad.getGeometry().equals(postLoad.getGeometry());
        //assert(equivalent);
        
        String [] preContents  = preLoad.toString().split("\\s");
        String [] postContents = postLoad.toString().split("\\s");
        //assert(preContents.length==postContents.length);
        System.out.printf("%d==%d\n",preContents.length,postContents.length);
        
        i = 0;
        for(String s: preContents){
            String print = String.format("'%s'=='%s'\n",s,postContents[i]);
            System.out.println(print);
            if(s.equals(postContents[i]))i++;
        }
        System.out.println(i);
        //if(equivalent) System.out.println("LOAD SUCCESSFUL");
    }
}
